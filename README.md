...
| --- | --- | --- | --- |
| **v5.7** | **v5.7（9 類）** | **交付權重**（`last.pt`） | **0.861 / 0.641**　⚠ 見下方口徑說明 |
| v5.7 ext | v5.7_ext（9 類） | 判準未通過 → 外部影像無效，**不交付** | 0.865 / 0.650 |
| v11.5 | v5.6（9 類） | 前一個交付基準。九類 ±2SE ≤ 0.10 達成 | 0.810 / 0.607（`last.pt`） |
| v12s | v5.6（9 類） | 判準未通過 → 容量不是瓶頸 | 0.813 / 0.618（`last.pt`） |
| v11 | v5.6（9 類） | `patience=30`，valid 不能併計 | 0.822 / 0.613 |
| v10 | v5.5（9 類） | 主要產出是發現評估集太小 | 0.809 / 0.606 |
| v9 / A0 | v5r（8 類） | **數字已作廢**（v5r 的評估集有 44.5%／47.5% 落在 v5 的 train 裡） | 0.891 / 0.678 |

> ⚠ **`0.861` 不可以拿去跟 `0.810` 比。** v5.7 只改了一件事：`Thrips_Damage`
> 的框定義換成「一張葉子一個框」，框的中位面積變成 **1.36 倍**，而 AP50 只要
> IoU ≥ 0.5 就算命中——框愈大愈容易達標。那 +0.05 幾乎全來自這一類。
> **標註未變動的八類平均是 0.85268 → 0.85082（−0.002），等於沒變。**
> 跨版本比較一律用八類平均。詳見
> [docs/v5.7_報告_交付與研究限制.md](docs/v5.7_報告_交付與研究限制.md) §2。

**換到 v5.7 的理由不是模型變好，是數字終於有解釋力。** `Thrips_Damage` 在 v5.6
的框彼此矛盾（兩人中位 IoU 只有 0.746），那個類別的 AP 量的是標註噪聲；
v5.7 的框照一份驗證過 **0.899** 的書面約定重畫，per-class 數字才守得住。
連帶地，從 v9 就掛著、三輪都沒隨評估集擴大而收斂的 valid/test 矛盾
（0.285 → 0.190 → 0.202）**一次就解除**（−0.202 → +0.121，門檻 0.165）。

### 四條路都收斂了

| 方向 | 結論 | 依據 |
| --- | --- | --- |
| **模型端** | 連四輪、十一種改動全部無效 | v9 六臂零臂過 2σ；v12s 的 3.84× 參數只換到 +0.003 |
| **訓練協定** | **有效**，但修的是「報得出來的精度」 | `patience=0` + `last.pt` 讓 ±2SE 從 0.140 降到 0.099 |
| **部署端** | **即時辨識不採用**，App 只做拍照辨識 | 五個候選延遲全過、精度全未過（最高 0.709 / 門檻 0.764） |
| **資料端** | 外部影像無效；重標**有效但只修掉一個假問題** | ext +0.021 / 門檻 0.04；重標後八類平均 −0.002 |

**在不新拍照片的前提下，工程手段已經用盡。** 兩個「有效」的都是讓數字變得可信，
沒有一個讓模型變好。每一項的判準與實測數字見
**[docs/通用_參考_已否決方向與依據.md](docs/通用_參考_已否決方向與依據.md)**。

剩下有依據的下一步只有兩件：量 `Scale_Insect` 的框一致性（它接手成為最弱的一類，
pooled 0.632，且 valid/test 落差在同一份資料上會從 0.04 跳到 0.17）、或補拍同域照片。

> **訓練協定**：一律 `patience=0` ＋ 固定輪數 ＋ 報 `last.pt`。
> v11.5 實測 `best.pt` 與 `last.pt` 差距 ≤ 0.004 且方向不一致，
> 改用 `last.pt` 沒有代價，卻讓 valid 可合法與 test 併計。
>
> **per-class 的雜訊地板是 ±0.04**，整體 mAP50 的 run 間全距是 **0.021**。
> 小於這個幅度的差異一律不解讀。
