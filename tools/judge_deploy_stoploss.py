...
與 compare_arms.py 同一個用意：**門檻寫死在程式裡，看結果之前就定好**。
  ...
  停損   S1 有組合過線 → 停止搜尋，交付其中 mAP50 最高者
         S2 第一階段量完仍沒有組合還有機會（或第二階段那一次重訓仍未過）→ 結案
         S3 過了 2026-09-22 → 無論進度如何都結案
...
def main() -> int:
    ...
    if passed:
        ...
        print(f"  S1 觸發：{len(passed)} 組過線 → 停止搜尋。交付 mAP50 最高的一組："
              f"{best['model']}（{best['lat_ms']:.1f} ms / mAP50 {best['mAP50']:.3f}）")
    elif today > DEADLINE:
        print(f"  S3 觸發：已過期限 {DEADLINE} → 部署線結案，交付 fp32@320（計畫書 §5）")
    else:
